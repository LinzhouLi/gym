...
export class Viewer {
  constructor(el, options) {
    ...
    this.playerNameBase64 = window.btoa(
      encodeURIComponent(
        this.options && this.options.playerName ? this.options.playerName : "aa"
      )
    );

    this.vrSpaceId = paramJson.space ? paramJson.space : "0";
    this.vrRoomId = paramJson.room ? paramJson.room : "100";
    this.vrCharId = paramJson.char
      ? paramJson.char
      : paramJson.gender
      ? paramJson.gender
      : 1;
    ...
    this.SetupNetwork = function () {
      this.mmoNetwork.Setup(
        this.playerNameBase64,
        "111111",
        this.vrSpaceId + "-" + this.vrRoomId,
        { avatar: this.vrCharId },
        (result) => {
          console.log(_self.playerAssets);

          _self.mainPlayer = new MMOPlayer(
            this.mmoNetwork,
            "player",
            _self.playerAssets,
            function (playerRenderableObject) {
    ...
    this.onKeyDown = function (event) {
      ...
      if (event.keyCode == 32) {
        ...
        _self.spawnPoints.push(
          new Vector3().copy(_self.mainPlayer.GetRenderableObject().position)
        );
